---
title: "Internet para clases en línea en México: qué necesitas (2026)"
slug: "mejor-internet-clases-en-linea-mexico"
date: 2026-08-09
description: "Velocidad de internet para clases en línea (Zoom, Meet, Teams): Mbps necesarios, fibra vs cable, qué hacer si se congela el video y opciones económicas."
draft: false
image: "/images/generated/home-office-cdmx.webp"
---

## Respuesta Rápida

**Para clases en línea en México 2026 necesitas 25-50 Mbps de bajada y 5-10 Mbps de subida, preferentemente por fibra. Totalplay 100 Mbps ($499 MXN) o Telmex Infinitum 200 Mbps ($599 MXN) son opciones económicas y estables. Evita horas pico (7-10 PM) si tienes problemas de saturación.**

- **Zoom/Meet/Teams consume ~5 Mbps por llamada en HD; 25 Mbps soportan 3 clases simultáneas sin problema.**
- **Conecta la laptop por cable Ethernet durante la clase y desactiva actualizaciones automáticas y torrents.**

Para más detalle, consulta [calcular Mbps necesarios](/guias/cuantos-mbps-necesito.html).

::: container
::: breadcrumbs
[Inicio](/) / [Blog](/blog.html) / Internet para clases en línea
:::

Para clases en línea necesitas estabilidad más que megas. Zoom y Google Meet funcionan con velocidades modestas (3-5 Mbps), pero si la conexión es inestable o la subida es muy baja, el video se congela. Aquí está qué internet necesitas para que las clases no se corten.

::: {.ad-slot ad-slot="after-intro"}
:::

::: section
## Mbps necesarios para plataformas de clases

| Plataforma | Descarga mínima | Subida mínima | Recomendado |
|---|---:|---:|---:|
| Zoom (video HD) | 3 Mbps | 3 Mbps | 5+ Mbps |
| Google Meet | 3 Mbps | 3 Mbps | 5+ Mbps |
| Microsoft Teams | 2 Mbps | 2 Mbps | 4+ Mbps |
| Plataforma escolar web | 2 Mbps | 1 Mbps | 5+ Mbps |

La subida es crítica: si tu plan tiene 2 Mbps de subida, una videollamada lo va a consumir todo. Si alguien más en casa usa internet a la vez, se congela.

Para entender las unidades, checa [la diferencia entre Mbps y MB](/guias/diferencia-mbps-vs-mb.html).
:::

<!-- ADSENSE_BREAK -->

::: section
## Fibra vs cable para clases en línea

| Aspecto | Fibra óptica | Cable coaxial |
|---|---|---|
| Estabilidad | Alta | Media-baja en horario pico |
| Velocidad de subida | Alta (ideal para video) | Baja (puede congelar) |
| Latencia | Baja | Media |
| Mejor para clases | Sí | Aceptable si subida ≥ 3 Mbps |

La fibra es la mejor opción para clases en línea porque tiene mejor velocidad de subida (lo que envía tu video al profesor) y menor latencia. Si tienes cable coaxial con 2 Mbps de subida, las videollamadas pueden congelarse.

Para entender más, lee [fibra vs coaxial](/internet-en-casa/fibra-vs-coaxial-mexico.html).
:::

::: {.ad-slot ad-slot="mid-content"}
:::

<!-- ADSENSE_BREAK -->

::: section
## Qué hacer si el video se congela

1. **Conecta por cable Ethernet** tu computadora o laptop (un adaptador USB cuesta $100-$200 MXN).
2. **Cierra otros programas** que consuman internet (Netflix, descargas, actualizaciones).
3. **Pide a otros en casa** que no usen streaming mientras estás en clase.
4. **Reduce la calidad del video** en Zoom/Meet si tu conexión es limitada.
5. **Siéntate cerca del router** si usas Wi-Fi (banda 5 GHz).
6. **Reinicia el router** 15 minutos antes de la clase.
7. **Activa QoS** en el router para priorizar videollamadas.

Para más diagnóstico, lee [velocidad necesaria para videollamadas](/blog/velocidad-necesaria-videollamadas-mexico-2026.html).
:::

::: section
## Opciones económicas para estudiantes

| Opción | Precio | Velocidad | Mejor para |
|---|---:|---|---|
| Plan fibra básico compartido | $300-$400 MXN/mes | 100-200 Mbps | Estudiantes viviendo con roomies |
| Internet sin contrato (4G fijo) | $200-$350 MXN/mes | 10-40 Mbps | Uso temporal o sin contrato |
| Plan móvil con hotspot | $150-$300 MXN/mes | Variable | Emergencias o uso muy ligero |
| WiFi de la escuela | $0 | Variable | Estudio en campus |

Para más opciones, lee [internet para estudiantes en México](/internet-en-casa/internet-para-estudiantes-mexico.html).
:::

::: section
## Preguntas frecuentes

### ¿Cuántos Mbps necesito para Zoom?

Mínimo 3 Mbps de descarga y 3 Mbps de subida. Para HD, 5+ Mbps. La estabilidad importa más que los megas.

### ¿Por qué mi video se ve bien pero el del profesor se congela?

Probablemente tu velocidad de subida es baja. El profesor te ve mal porque tu video no sale bien.

### ¿Sirve el internet del celular para clases?

Para emergencias sí, pero el hotspot consume datos rápido y la latencia puede causar cortes. No es ideal para uso diario.

### ¿Cable o WiFi para clases?

Siempre cable Ethernet si puedes. Un adaptador USB-Ethernet cuesta $100-$200 MXN y elimina el 90% de los cortes.

Para más sobre esto, lee [cómo mejorar el Wi-Fi en casa](/internet-en-casa/como-mejorar-wifi-casa-mexico.html).
:::

::: {.section related-block="" style="margin-top:18px"}
## Sigue aprendiendo

- Para medir tu velocidad real, checa [cómo hacer un test de velocidad](/guias/como-medir-velocidad-real-internet.html).
- Para entender qué velocidad necesitas, lee [cuánta velocidad necesitas](/blog/cuanta-velocidad-internet-necesitas-2026.html).
- Para optimizar tu conexión, revisa [cómo mejorar el Wi-Fi en casa](/internet-en-casa/como-mejorar-wifi-casa-mexico.html).
:::

::: meta-row
**Última actualización:** agosto de 2026 • [Metodología](/metodologia.html)
:::
:::
